...
class BedrockSummarizer:
    ...
    The helper methods `_build_request_body` and `_extract_text` make this class
    model-agnostic: they map your prompt into the correct schema for each family
    (Amazon Nova vs. Anthropic Claude) and read the reply no matter which schema
    is returned.
    """
    ...
    def _format_clusters_for_prompt(clusters: List[LogCluster]) -> str:
        ...
        for cl in sorted_clusters:
            tag = " (RECURRING)" if cl.is_recurring else ""
            lines.append(f'- Signature: "{cl.signature}", Occurrences: {cl.count}{tag}')
        return "\n".join(lines)

    @staticmethod
    def generate_fallback_summary(clusters: List[LogCluster]) -> str:
        """
